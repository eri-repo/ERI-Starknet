...
import {Contract, RpcProvider, Abi, ContractAddress, Provider, Account} from "starknet";
...
import {getTypedDataHash, Certificate} from "../resources/certificateData";
import {
    getCertificateTypedDataHash,
    //   Certificate,
} from "../resources/typedDataHash";
...
export default function App() {
    ...
    // @ts-ignore
    const getContract = async (contractAddress: ContractAddress, contractType: number): Promise<Contract> => {
    ...
    const verifyStructArray = async (e: React.FormEvent) => {
        ...
        // console.log("ABI: ", JSON.stringify(abi as Abi, null, 2));

        try {
            // const contract = new Contract(STRUCT_ABI, CONTRACT_ADDRESS, account);

            const contract = await getContract(CONTRACT_ADDRESS, 1);
